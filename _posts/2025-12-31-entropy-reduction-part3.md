...
*图1：长序列退化不是熵增，而是轨道失稳——从稳定区经过扰动进入漂移区，最终指数发散*
...
*图2：模型轨道——从初始状态出发，经过状态1、2、3...一步步演化形成的路径*
...
*图3：思维链（Chain-of-Thought）将推理过程展开成Token序列，例如"5-2+3=6"的计算过程*
...
*图4：显式推理（上）在Token空间反复投影，每步都有投影损失；隐空间推理（下）在高维空间连续演化，无需投影*
...
*图5：Token层面修正需要额外tokens且用户可见；隐空间修正连续进行，对用户无感且实时连续*
...
*图6：Superposition现象——高维隐空间中特征A、B、C、D正交干净分离，但投影到单神经元激活后混叠不清*
